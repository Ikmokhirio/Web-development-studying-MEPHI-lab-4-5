    ...
    function (req, username, password, done) {
        ...
        isUserExist(username).then(res => {
            if (res) {
                ...
                createNewUser(username, password, req.body.phone_number, req.body.gender, req.body.description,'Admin').then(newUser => {
...
const apiStrategy = new CookieStrategy({
    cookieName: cookieName,
    passReqToCallback: true
}, function (req, session, done) {
    if (!req.user) return done(null, false, {message: "You should authorize to access this page"});
    ...
    findUser(req.user.username).then(user => {
        if (user !== undefined && user !== null) {
            if(user.role === 'Admin') {
                ...
                return done(null, false, {message: "You do not have permission for this"});
            }
